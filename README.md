# DerStandard Enhancer

A local [Violentmonkey](https://violentmonkey.github.io/) userscript for discovery, reading progress, and comment browsing on [derStandard.at](https://www.derstandard.at/).

## Install

1. Install [Violentmonkey](https://chromewebstore.google.com/detail/violentmonkey/jinjaccalgkegednnccohejagnlnfdag).
2. In Brave, open `brave://extensions`, open Violentmonkey's **Details**, and enable **Allow User Scripts**.
3. In Violentmonkey, choose **Install from URL** and paste:

   `https://raw.githubusercontent.com/glockyco/derstandard-enhancer/main/derstandard-enhancer.user.js`

4. Reload derStandard.at.

The userscript declares GitHub `@downloadURL` and `@updateURL` metadata for future updates.

## Features

- Current-page, newest-RSS, and locally observed most-commented discovery.
- Search plus read, saved, and ignored filters.
- Visited markers, scroll progress, resume reading, and JSON export/import.
- Font scaling, text-width preferences, and an article outline.
- Reversible sorting of loaded top-level comments by rating.
- Keyboard shortcuts: `Alt+Shift+O` opens the panel; `Alt+Shift+R` resumes reading.
- Local-only state via `localStorage`; no posting, voting, private API calls, or external sync.

“Most commented” uses counts visible on the current page, not a global ranking API. Comment sorting affects loaded top-level comments only; RSS discovery falls back safely when unavailable.

## Development

[`src/`](src/) contains the source modules. [`derstandard-enhancer.user.js`](derstandard-enhancer.user.js) is the bundled installable file.
